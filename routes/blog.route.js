...
router.post('/blogs', verifyToken, verifyRole('Manager'),upload.single('thumbnaiUrl'),createBlog)
router.get('/blogs', verifyToken, verifyRole('Manager'),getAllBlogs)
router.get('/blogs/:id',viewDetailBlogs)
router.patch('/blogs/:id', verifyToken, verifyRole('Manager'),upload.single('thumbnaiUrl'),updateBlog)
